...
# -*- coding: utf-8 -*-
"""
将 SFT 数据集分割为训练集、验证集和测试集

使用方法：
    python split_sft_dataset.py
...
# 初始化日志
log = Logger('split_sft_dataset', std_out=True, save2file=True, file_name=None)
...
def split_dataset(
    ...
    test_size = total_size - train_size - valid_size  # 剩余部分分配给测试集
    ...
    log.info("")
    log.info("分割结果:")
    log.info(f"  训练集: {len(train_df)} 行 ({len(train_df)/total_size*100:.2f}%)")
    log.info(f"  验证集: {len(valid_df)} 行 ({len(valid_df)/total_size*100:.2f}%)")
    log.info(f"  测试集: {len(test_df)} 行 ({len(test_df)/total_size*100:.2f}%)")
